...
class WeatherApp:
    ...
    def display_weather_icon(self, icon_id):
        ...
        self.icon_label.image = (
            icon_photo  # Keep a reference to avoid garbage collection
        )
    ...
    def create_gui(self):
        ...
        title_label = ttk.Label(
            frame,
            text="OASIS INFOBYTE WEATHER APPLICATION",
            font=("Arial", 18, "bold"),
            style="TLabel",
        )
        ...
        # Input Section
        ttk.Label(frame, text="Enter City:", style="TLabel").grid(
            row=1, column=0, sticky=tk.W, pady=(0, 5)
        )
        self.city_entry = ttk.Entry(frame, width=20)
        self.city_entry.grid(row=1, column=1, sticky=tk.W, pady=(0, 5))

        ttk.Label(frame, text="Select Unit:", style="TLabel").grid(
            row=2, column=0, sticky=tk.W, pady=(0, 5)
        )
        unit_dropdown = ttk.Combobox(
            frame, textvariable=self.selected_unit, values=self.temperature_units
        )
        unit_dropdown.grid(row=2, column=1, sticky=tk.W, pady=(0, 5))

        get_weather_button = ttk.Button(
            frame, text="Get Weather", command=self.get_weather_data, style="TButton"
        )
